...
const tuples: [string, number] = ["Davdi", 10];
const array: string[] = ["David", "Diana", "Harry"];
...
// ) {
//   let result: number | string;
//   if (typeof (input1) === "number" && typeof (input2) === "number" || (resultType) === "as-number" || "as-text") {
//     result = +input1 + +input2;
//     return result;
//   } else {
//     result = input1.toString() + input2.toString();
...
// //   }
// // }
// console.log(combination("8", "10", "as-number"));
// console.log(combination("Petr", "10", "as-text"));
// console.log(combination("Petr", "Pavel", "number"));
...
function houseDescription(myObject: any) {
  console.log(
    `Tento sům má ${myObject.windows} okna. Počet dveří je ${myObject.doors}. Barva domu je ${myObject.color}. Výška domu je ${myObject.height} metrů`
  );
}
...
const x = (nejakeCislo: number) => console.log(nejakeCislo);

function sum1(n1: number, n2: number, callBackFun: (n3: number) => void) {
  const result = n1 + n2;
  callBackFun(result);
}
...
let result1;
function sums(n1: number, n2: number) {
  result1 = n1 + n2;
  return result1;
}
...
function sumBig(n1: number, n2: number) {
  return n1 + n2;
}
...
const mySum1 = (n1: number = 5, n2: number = 2) => n1 + n2;
...
const employesss: string[] = ["David", "Harry", "Hermiona", "Ron"];
...
class House {
  ...
  constructor(st: string, nu: number, flo: number, sta: string) {
    this.street = st;
    this.number = nu;
    this.floors = flo;
    this.state = sta;
...
class ItDepartment extends Department1 {
  ...
  constructor(string: string = "IT", number: number, public admins: string[]) {
    super(string, number);
    this.admins = admins;
...
// interface, vytváříme interface pro druhy hodnot
...
// interface a class a readonly(Můžete vytvořit jenom jednou)
// volitelné parametry a metody u interfacu  age?: number volitelné nemusí být obsaženo v objektu

interface AddFunction {
  (a: number, b: number): number;
}

let sumTest: AddFunction;
sumTest = (number1: number, number2: number) => {
...
let person2: IamGreeting = new Person("Petr");
console.log(person2);
